		...
		/*WARNING: the Exselector project reserves the right to
		//modify this code, IOW it's not verbatim nanovg code*/
...
enum GLNVGshaderType {
	...
	NSVG_SHADER_FILLIMG,
	NSVG_SHADER_SIMPLE,
	NSVG_SHADER_IMG //text
};

//#if NANOVG_GL_USE_UNIFORMBUFFER
enum GLNVGuniformBindings {
	GLNVG_FRAG_BINDING = 0, ///////////////THIS WILL BE A PROBLEM/////////////////
#define GLNVG_FRAG_BINDING2 "0"
	GLNVG_VERT_BINDING = 1, //2022
#define GLNVG_VERT_BINDING2 "1"
};
//#endif
...
struct GLNVGvertUniforms { //2022	
	float viewMat[4*4]; //world for VR
	};
struct GLNVGfragUniforms {
	#if 1//NANOVG_GL_USE_UNIFORMBUFFER
...
typedef struct GLNVGvertUniforms GLNVGvertUniforms;
...
#endif
	*/
	//I don't want to fool with separation
	assert(blend->srcRGB==blend->srcAlpha&&blend->dstRGB==blend->dstAlpha);
	...
	//	(gl->blendFunc.srcAlpha != blend->srcAlpha) ||
		...
		gl->blendFunc = *blend;
		//glBlendFuncSeparate(blend->srcRGB, blend->dstRGB, blend->srcAlpha,blend->dstAlpha);
		glBlendFunc(blend->srcAlpha,blend->dstAlpha);
	}
#else
	//glBlendFuncSeparate(blend->srcRGB, blend->dstRGB, blend->srcAlpha,blend->dstAlpha);
	glBlendFunc(blend->srcAlpha,blend->dstAlpha);
...
#ifndef glnvg__checkError //glDebugMessageCallback?
...
//		"	uniform vec2 viewSize;\n"
		"	layout(binding=" GLNVG_VERT_BINDING2 ",std140) uniform vert {\n" ///////// TODO: binding NEEDED ///////// GLNVG_FRAG_BINDING
		"		mat4 viewMat;\n"
		"	};\n"
		"	in vec2 vertex;\n"
		"	in vec2 tcoord;\n"
...
//		"	gl_Position = vec4(2.0*vertex.x/viewSize.x - 1.0, 1.0 - 2.0*vertex.y/viewSize.y, 0, 1);\n"
		"	gl_Position = viewMat*vec4(vertex,1.0,1.0);\n"
		"}\n";

	static const char* fillFragShader =
	
		"#define type 3\n" //always text (optimizing) //NSVG_SHADER_IMG 
...
//		"#ifdef USE_UNIFORMBUFFER\n"
		"	layout(binding=" GLNVG_FRAG_BINDING2 ",std140) uniform frag {\n" ///////// TODO: binding NEEDED ///////// GLNVG_FRAG_BINDING
		"		mat3 scissorMat;\n"
		"		mat3 paintMat;\n"
		"		vec4 innerCol;\n"
		"		vec4 outerCol;\n"
...
//	glUniformBlockBinding(gl->shader.prog, gl->shader.loc[GLNVG_LOC_FRAG], GLNVG_FRAG_BINDING);
	glGenBuffers(1, &gl->fragUBuf);
	glGenBuffers(1, &gl->vertUBuf);
	glGetIntegerv(GL_UNIFORM_BUFFER_OFFSET_ALIGNMENT, &align);
//#endif
	gl->fragUSize = sizeof(GLNVGfragUniforms) + align - sizeof(GLNVGfragUniforms) % align;
	gl->vertUSize = sizeof(GLNVGvertUniforms) + align - sizeof(GLNVGvertUniforms) % align; //2022

//Nvidia glBufferData performance is terrible
glBindBuffer(GL_UNIFORM_BUFFER,gl->fragUBuf);
glBufferData(GL_UNIFORM_BUFFER,gl->fragUSize, 0, GL_STREAM_DRAW);
glBindBuffer(GL_UNIFORM_BUFFER,gl->vertUBuf);
glBufferData(GL_UNIFORM_BUFFER,gl->vertUSize, 0, GL_STREAM_DRAW);

glBindBufferRange(GL_UNIFORM_BUFFER, GLNVG_VERT_BINDING, gl->vertUBuf, 0, gl->vertUSize);
...
static int glnvg__renderCreateTexture(void* uptr, int type, int w, int h, int imageFlags, const unsigned char* data)
{
	...
	//REMINDER: mipmapping destroys the signal
	int min = 0?GL_LINEAR_MIPMAP_LINEAR:GL_LINEAR;
...
static void glnvg__setUniforms(GLNVGcontext* gl, int uniformOffset, int image)
{
	GLNVGtexture* tex = NULL;
#if 1//NANOVG_GL_USE_UNIFORMBUFFER
//	glBindBufferRange(GL_UNIFORM_BUFFER, GLNVG_FRAG_BINDING, gl->fragUBuf, uniformOffset, sizeof(GLNVGfragUniforms));
	glBindBufferRange(GL_UNIFORM_BUFFER, GLNVG_FRAG_BINDING, gl->fragUBuf, uniformOffset, gl->fragUSize);
#else
...
static void glnvg__renderView(void* uptr, float mat[4*4])
{
	int compile[sizeof(float)*4*4==sizeof(GLNVGvertUniforms)]; (void)compile;

	GLNVGcontext* gl = (GLNVGcontext*)uptr;

	glBindBuffer(GL_UNIFORM_BUFFER, gl->vertUBuf);
 //	glBufferData(GL_UNIFORM_BUFFER, sizeof(GLNVGvertUniforms), mat, GL_STREAM_DRAW);
	glBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(GLNVGvertUniforms), mat);
}
...
static void glnvg__fill(GLNVGcontext* gl, GLNVGcall* call)
{
	...
	glnvg__setUniforms(gl, call->uniformOffset + gl->fragUSize, call->image);
...
//		glnvg__convertPaint(gl, nvg__fragUniformPtr(gl, call->uniformOffset + gl->fragUSize), paint, scissor, strokeWidth, fringe, 1.0f - 0.5f/255.0f);
...
static GLNVGblend glnvg__blendCompositeOperation(NVGcompositeOperationState op)
{
	...
	{
//NOTE: changing to NVG_SRC_ALPHA dosen't match ID3DXFont look
//(nvgReset changes back to NVG_SOURCE_OVER, so overriding here
//doesn't seem like it sticks)
		blend.srcRGB = GL_ONE; 
...
static void glnvg__renderFlush(void* uptr, int(*xr_f)(int))
{
	GLNVGcontext* gl = (GLNVGcontext*)uptr;
	int i;

//	if (gl->ncalls > 0) //guaranteed state?
	{
		// Setup require GL state.
		glUseProgram(gl->shader.prog);
//		glEnable(GL_CULL_FACE);
glDisable(GL_CULL_FACE); //TODO: seems can reverse draw order?
//		glCullFace(GL_BACK);
//		glFrontFace(GL_CCW);
		glEnable(GL_BLEND);
//glDisable(GL_BLEND); //TESTING
//		glDisable(GL_DEPTH_TEST);
//		glDisable(GL_SCISSOR_TEST); //???
//		glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE); //???
//		glStencilMask(0xffffffff); //default
//		glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP); //default
//		glStencilFunc(GL_ALWAYS, 0, 0xffffffff); //default
...
//#if NANOVG_GL_USE_UNIFORMBUFFER
		...
		glBindBuffer(GL_UNIFORM_BUFFER, gl->fragUBuf);
		//Nvidia glBufferData performance is terrible
		//glBufferData(GL_UNIFORM_BUFFER, gl->nuniforms * gl->fragUSize, gl->uniforms, GL_STREAM_DRAW);
glBufferSubData(GL_UNIFORM_BUFFER, 0,gl->nuniforms * gl->fragUSize, gl->uniforms);
//#endif
...
//TODO: can do this at creation time
...
//2022: I don't think this is doing anything for GLES3 (glBindBufferRange)
//		glBindBuffer(GL_UNIFORM_BUFFER, gl->fragUBuf);

//#endif

		auto *f = xr_f; for(int v=0;;v++) //OpenXR?
		{
			if(f&&!f(v)) break;
...
//#endif
//		glDisable(GL_CULL_FACE);
glEnable(GL_CULL_FACE); //TODO: seems can reverse draw order?
...
static int glnvg__allocVerts(GLNVGcontext* gl, int n)
{
	...
	//Nvidia glBufferData performance is terrible
glBindBuffer(GL_ARRAY_BUFFER,gl->vertBuf);
glBufferData(GL_ARRAY_BUFFER,gl->cverts * sizeof(NVGvertex), 0, GL_STREAM_DRAW);
...
{
	int ret = 0, structSize = gl->fragUSize;
	...
	//Nvidia glBufferData performance is terrible
glBindBuffer(GL_UNIFORM_BUFFER,gl->fragUBuf);
glBufferData(GL_UNIFORM_BUFFER,structSize*cuniforms, 0, GL_STREAM_DRAW);
							  ...
							  const float* bounds, const NVGpath* paths, int npaths)
{
	...
	if (call->type == GLNVG_FILL) {
		...
		// Fill shader
		glnvg__convertPaint(gl, nvg__fragUniformPtr(gl, call->uniformOffset + gl->fragUSize), paint, scissor, fringe, fringe, -1.0f);
	} else {
...
//#if NANOVG_GL_USE_UNIFORMBUFFER
	if (gl->fragUBuf != 0)
		glDeleteBuffers(1, &gl->fragUBuf);
if (gl->vertUBuf != 0)
		glDeleteBuffers(1, &gl->vertUBuf);
//#endif
	if (gl->vertArr != 0)
...
params.renderView = glnvg__renderView;
