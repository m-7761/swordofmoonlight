...
//#define EX_NVG_SCALE //NOTHING (TESTING)
...
#include "stb_image.h"
	...
	#endif //__STB_INCLUDE_STB_TRUETYPE_H__ //nanovg.pch.h
...
//4096 is needed for large VR fonts to avoid
//regenerating every frame
#define NVG_INIT_FONTIMAGE_SIZE  1024 //512
#define NVG_MAX_FONTIMAGE_SIZE   8192 //2048
...
//void nvgBeginFrame(NVGcontext* ctx, float viewMat[4*4], float devicePixelRatio)
...
//	ctx->params.renderView(ctx->params.userPtr, viewMat);
...
void nvgView(NVGcontext* ctx, float viewMat[4*4])
{
	//NOTE: passing a 3D view into here is probably not
	//good for 2D SVG effects
	ctx->params.renderView(ctx->params.userPtr,viewMat);
}
...
void nvgEndFrame(NVGcontext* ctx, int(*xr_f)(int))
{
	ctx->params.renderFlush(ctx->params.userPtr,xr_f);
