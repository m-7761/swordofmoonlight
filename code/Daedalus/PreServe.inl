...
inline pre3D &Pre3D::operator+=(const pre3D &o){ x+=o.x; y+=o.y; z+=o.z; return *this; }
inline pre3D &Pre3D::operator-=(const pre3D &o){ x-=o.x; y-=o.y; z-=o.z; return *this; }
inline pre2D &Pre2D::operator*=(double s){ x*=s; y*=s; return *this; }
inline pre3D &Pre3D::operator*=(double s){ x*=s; y*=s; z*=s; return *this; }
inline pre4D &Pre4D::operator*=(double s){ x*=s; y*=s; z*=s; w*=s; return *this; }
...
inline bool Pre4D::operator==(const pre4D &o)const{ return x==o.x&&y==o.y&&z==o.z&&w==o.w; }
inline bool Pre3D::operator!=(const pre3D &o)const{ return x!=o.x||y!=o.y||z!=o.z; }
inline bool Pre4D::operator!=(const pre4D &o)const{ return x!=o.x||y!=o.y||z!=o.z||w!=o.w; }
...
inline pre2D operator*(const pre2D &v, double s){ return pre2D(s*v.x,s*v.y); }
inline pre3D operator*(const pre3D &v, double s){ return pre3D(s*v.x,s*v.y,s*v.z); }
inline pre4D operator*(const pre4D &v, double s){ return pre4D(s*v.x,s*v.y,s*v.z,s*v.w); }
inline pre2D operator/(const pre2D &v, const pre2D &v2){ return pre2D(v.x/v2.x,v.y/v2.y); }
...
DAEDALUS_CALLMETHOD void PreMaterial::GetTextureDimensions
(preN (&tc)[PreMorph::texturecoordslistsN], preN (&vc)[PreMorph::colorslistsN]
,const Property *(PreMaterial::*FindOrPairKey)(decltype(mapping),Texture,preN)const)const
{
	memset(tc,0x00,sizeof(tc)); memset(vc,0x00,sizeof(vc));
	auto il = PropertiesList();
	for(preN i=il;i<il.Size();i++) if(il[i]->key==_sourceslist)
	{	
		const Property *kv = il[i];	
		preN *paranoia = 0, d = 0, n = kv->Datum(sourceslist);
		kv = (this->*FindOrPairKey)(mapping,kv->TextureCategory(),kv->TextureNumber());
		if(kv) switch(kv->Datum(mapping))
		{
		case texturecoords3D: d++; case texturecoords2D: d++; case texturecoords1D: d++;
		if(n<PreMorph::texturecoordslistsN) paranoia = tc+n; else assert(0); break;
		case colors4D: d++;	case colors3D: d++;	case colors2D: d++;	case colors1D: d++;
		if(n<PreMorph::colorslistsN) paranoia = vc+n; else assert(0); break;
		}if(paranoia) *paranoia = std::max<preN>(d,*paranoia);
	}
}
